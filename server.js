...
app.get("/login",passport.authenticate("local", { failureRedirect: "log" }),
    (req, res) => {
        logger.info(`Usuario ${username} en home principal`);
        res.redirect("/home");
    }
);
...
app.get("/logout", (req, res) => {
    logger.info(`Usuario ${username} finalizó session`);
...
app.get("/info", (req, res) => {
    //console.log("SERVER INFO TEST ARTILLERY");
...
app.get("*", (req, res) => {
    ...
    logger.warn(`Ruta ${url} con metodo ${method} no implementada`);
    res.send(`Ruta ${url} con metodo ${method} no implementada`);
});
...
if (modoCluster && cluster.isPrimary) {
    ...
    server.on("info", (info) => console.log(`Info en servidor: ${info}`));
    server.on("warning", (warning) => console.log(`Warning en servidor: ${warning}`));
  }
